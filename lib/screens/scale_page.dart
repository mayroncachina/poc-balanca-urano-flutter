...
class _ScalePageState extends State<ScalePage> {
  ...
  List<List<int>> _extractFramesFromBuffer(List<int> buffer) {
    final frames = <List<int>>[];
    while (true) {
      final stx = buffer.indexOf(0x02);
      if (stx < 0) break;
      final etx = buffer.indexOf(0x03, stx + 1);
      if (etx < 0) {
        if (stx > 0) buffer.removeRange(0, stx);
        break;
      }
      final frame = buffer.sublist(stx, etx + 1);
      frames.add(List<int>.from(frame));
      buffer.removeRange(0, etx + 1);
    }
    if (buffer.length > 4096) buffer.removeRange(0, buffer.length - 1024);
    return frames;
  }

  String? _parseFramedWeight(List<int> frame) {
    final start = frame.indexOf(0x02);
    final end = frame.lastIndexOf(0x03);
  ...
  String? _safeAscii(List<int> bytes) {
    try {
      ...
      final nonPrintable =
          s.runes.where((c) => c < 32 && c != 9 && c != 10 && c != 13).length;
